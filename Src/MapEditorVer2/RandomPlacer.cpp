...
void RandomPlacer::SpawnObject(int model, bool flag)
{
    ...
    CFbxMesh* mesh = CModelStorage::GetInstance()->GetModel(model_names_[model]);
    ...
    for (int i = 0; i < count_; i++)
    {
        ...
        for (int retry = 0; retry < kMaxRetry; retry++)
        {
            ...
            Aabb candidate =
            {
                { x + mesh_min.x, y + mesh_min.y, z + mesh_min.z },
                { x + mesh_max.x, y + mesh_max.y, z + mesh_max.z }
            };

            bool overlap = false;
            for (const auto& a : placed)
            {
                if (candidate.min.x <= a.max.x && candidate.max.x >= a.min.x &&
                    candidate.min.y <= a.max.y && candidate.max.y >= a.min.y &&
                    candidate.min.z <= a.max.z && candidate.max.z >= a.min.z)
                {
                    overlap = true;
                    break;
                }
