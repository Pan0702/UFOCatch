    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        for (mi = 0; mi < m_dwMeshNum; mi++) // メッシュ数だけ繰り返し
        {
            ...
            if (m_nMeshType == 1) // スタティックメッシュ
            {
                ...
                m_pMeshArray[mi].m_vStaticVerticesNormal = new StaticVertexNormal[m_pMeshArray[mi].m_dwVerticesNum];
                // 頂点配列バンプマップ用を生成する
                p += sizeof(int);

                // 頂点データ
                memcpy_s(staticvertices, sizeof(StaticVertex) * m_pMeshArray[mi].m_dwVerticesNum, p,
                         sizeof(StaticVertex) * m_pMeshArray[mi].m_dwVerticesNum); // 頂点配列に頂点を得る
                ...
                memcpy_s(m_pMeshArray[mi].m_nIndices, sizeof(DWORD) * m_pMeshArray[mi].m_dwIndicesNum, p,
                         sizeof(DWORD) * m_pMeshArray[mi].m_dwIndicesNum); // インデックス配列にインデックスを得る
                ...
                ChangeStaticVertexLayout(staticvertices, m_pMeshArray[mi].m_nIndices, m_pMeshArray[mi].m_dwIndicesNum,
                                         m_pMeshArray[mi].m_vStaticVerticesNormal);
            ...
            else if (m_nMeshType == 2) // スキンメッシュ
            {
                ...
                m_pMeshArray[mi].m_vSkinVerticesNormal = new SkinVertexNormal[m_pMeshArray[mi].m_dwVerticesNum];
                // 頂点配列バンプマップ用を生成する
                p += sizeof(int);

                // 頂点データ
                memcpy_s(skinvertices, sizeof(SkinVertex) * m_pMeshArray[mi].m_dwVerticesNum, p,
                         sizeof(SkinVertex) * m_pMeshArray[mi].m_dwVerticesNum); // 頂点配列に頂点を得る
                ...
                // 頂点のレイアウトを変換し、接線と従法線を計算
                ChangeSkinVertexLayout(skinvertices, m_pMeshArray[mi].m_nIndices, m_pMeshArray[mi].m_dwIndicesNum,
                                       m_pMeshArray[mi].m_vSkinVerticesNormal);
...
// 
//------------------------------------------------------------------------
bool CFbxMesh::ChangeStaticVertexLayout(const StaticVertex* vertices, const DWORD* indices, const DWORD& IndicesNum,
                                        StaticVertexNormal* verticesNormal)
{
    ...
    for (int i = 0; i < IndicesNum / 3; i++)
    {
        ...
        CalcTangentSub(verticesNormal[indices[i * 3]].Pos, verticesNormal[indices[i * 3 + 1]].Pos,
                       verticesNormal[indices[i * 3 + 2]].Pos,
                       verticesNormal[indices[i * 3]].vTex, verticesNormal[indices[i * 3 + 1]].vTex,
                       verticesNormal[indices[i * 3 + 2]].vTex,
...
// 
//------------------------------------------------------------------------
bool CFbxMesh::ChangeSkinVertexLayout(const SkinVertex* vertices, const DWORD* indices, const DWORD& IndicesNum,
...
HRESULT CFbxMesh::SetTexture(const DWORD& ti, const TCHAR* TexName)
{
    ...
    if (m_pTextureArray[ti] == nullptr)
    {
        ...
        m_pTextureArray[ti] = m_pFbxMeshCtrl->SetTextureList(_T("data/error.png"));
    }
    if (m_pTextureArray[ti] == nullptr)
    {
        MessageBox(nullptr, TexName, _T("ErrorTextureがありません"), MB_OK);
...
void CFbxMesh::MakeRootAnimAndShaderMatrix(const int& animNum, ROOTANIMTYPE RAType)
{
    ...
    {
        MessageBox(nullptr, _T("■□■ MakeRootAnimAndShaderMatrix() ■□■"), _T("■□■ スキンメッシュに、指定されたアニメーションが読み込まれていません ■□■"),
                   MB_OK);
        return;
    }
    ...
    // ルートボーン関係の変数を設定
    m_RootBoneArray[animNum].bindPose = m_pMeshArray[RootMeshNo].m_BoneArray[animNum][RootBoneNo].bindPose;
    // ルートボーンのバインドポーズ
    m_RootBoneArray[animNum].framePoseNum = m_pMeshArray[RootMeshNo].m_BoneArray[animNum][RootBoneNo].framePoseNum;
    // ルートボーンのフレーム数
        ...
        case eRootAnimXZ:
            // ルートボーンのバインドポーズからの差分（ＸＺ移動のみ）の行列を作成する
            {
                ...
                m_RootBoneArray[animNum].framePose[n] =
                    m_RootBoneArray[animNum].bindPose * m_pMeshArray[RootMeshNo].m_BoneArray[animNum][RootBoneNo].
                    framePose[n];
    ...
    for (int mi = 0; mi < m_dwMeshNum; mi++) // メッシュ数だけ繰り返し
    {
        ...
        m_pMeshArray[mi].m_pBoneShader[animNum] = new BONESHADER[m_pMeshArray[mi].m_BoneArray[animNum][0].framePoseNum];
        // シェーダーボーン配列を生成する
        ...
        for (int n = 0; n < m_pMeshArray[mi].m_BoneArray[animNum][0].framePoseNum; n++) // 各フレームポーズ毎の処理
        {
            for (int bi = 0; bi < m_pMeshArray[mi].m_NumBones; bi++) // 各ボーン毎の処理
            {
                ...
                if (RAType == eRootAnimNone)
                {
                    ...
                    mFramePose = m_pMeshArray[mi].m_BoneArray[animNum][bi].bindPose * m_pMeshArray[mi].m_BoneArray[
                        animNum][bi].framePose[n];
                }
                else
                {
                    // ルートボーンアニメをするとき
                    mFramePose = m_pMeshArray[mi].m_BoneArray[animNum][bi].framePose[n] *
                        XMMatrixInverse(nullptr, m_RootBoneArray[animNum].framePose[n]);
                    // 各フレームポーズ行列をルートボーン行列との差分行列にする            // -- 2020.12.15 -- 3
                    mFramePose = m_pMeshArray[mi].m_BoneArray[animNum][bi].bindPose * mFramePose;
                    // バインドボーズの逆行列とフレームポーズを掛け合わせ、バインドボーズからの差分の行列を作成する            // -- 2020.12.15 -- 3
                }
...
//==========================================================================================================================================================
MATRIX4X4 CFbxMesh::GetFrameMatrices(Animator* animStatus, const MATRIX4X4& mWorld, const DWORD& nBone,
                                     const DWORD& nMesh)
{
    MATRIX4X4 mBoneWorld;
...
MATRIX4X4 CFbxMesh::GetRootAnimUpMatrices(Animator* animStatus, const int& UpFrame, const int& StartFrameUp)
{
    ...
    if (animStatus == nullptr || !m_Animation[animStatus->PlayingID()].used) // -- 2024.9.5
    {
        MessageBox(nullptr, _T("■□■ GetRootAnimUpMatrices() ■□■"), _T("■□■ スキンメッシュに、指定されたアニメーションが読み込まれていません ■□■"),
                   MB_OK);
        return XMMatrixIdentity();
    }
...
{
    RenderDisplace(mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir,
                   GameDevice()->m_vEyePt);
}

void CFbxMesh::Render(Animator* animStatus, const MATRIX4X4& mWorld)
{
    Render(animStatus, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir,
           GameDevice()->m_vEyePt);
}

void CFbxMesh::RenderDisplace(Animator* animStatus, const MATRIX4X4& mWorld)
{
    RenderDisplace(animStatus, mWorld, GameDevice()->m_mView, GameDevice()->m_mProj, GameDevice()->m_vLightDir,
                   GameDevice()->m_vEyePt);
}
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight,
                      const VECTOR3& vEye)
{
    if (m_nMeshType == 1)
    {
...
void CFbxMesh::RenderDisplace(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
                            const VECTOR3& vLight, const VECTOR3& vEye)
{
    //使用するシェーダーの登録	
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderDisplaceStatic(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
                                    const VECTOR3& vLight, const VECTOR3& vEye)
{
...
//
//------------------------------------------------------------------------
void CFbxMesh::Render(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
                      const VECTOR3& vLight, const VECTOR3& vEye)
{
    if (m_nMeshType == 1)
    {
...
void CFbxMesh::RenderDisplace(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView,
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderSkin(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
                          const VECTOR3& vLight, const VECTOR3& vEye)
{
    // メッシュの描画
...
//
//------------------------------------------------------------------------
void CFbxMesh::Draw(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj,
                    const VECTOR3& vLight, const VECTOR3& vEye)
{
...
//
//------------------------------------------------------------------------
void CFbxMesh::RenderDisplaceSkin(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView,
                                  const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
    // メッシュの描画
...
//
//------------------------------------------------------------------------
void CFbxMesh::DrawDisplace(Animator* animStatus, const MATRIX4X4& mWorld, const MATRIX4X4& mView,
                            const MATRIX4X4& mProj, const VECTOR3& vLight, const VECTOR3& vEye)
{
    if (animStatus == nullptr) return; // -- 2024.9.5
...
void CFbxMesh::SetShaderMatColor(const TCHAR ProfileName[], int SlotNo, int i)
{
    ...
    if (SUCCEEDED(
        m_pD3D->m_pDeviceContext->Map(m_pShader->m_pConstantBufferMaterial, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
    {
        // マテリアルカラーを渡す
