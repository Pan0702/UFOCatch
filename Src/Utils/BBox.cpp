...
bool CBBox::OBBCollisionDetection(CBBox* pOtherBBox, VECTOR3* vHit, VECTOR3* vNrm)
{
    ...
    mWorldCenterB = XMMatrixTranslation(pOtherBBox->m_fLengthX + pOtherBBox->m_vMin.x,
                                        pOtherBBox->m_fLengthY + pOtherBBox->m_vMin.y,
                                        pOtherBBox->m_fLengthZ + pOtherBBox->m_vMin.z);
...
//------------------------------------------------------------------------
bool CBBox::CompareLength(const CBBox* pBBoxA, const CBBox* pBBoxB, const VECTOR3* pvSeparate,
                          const VECTOR3* pvDistance)
{
...
// 戻り値　なし
//------------------------------------------------------------------------
void CBBox::Render(const MATRIX4X4& mWorld, const MATRIX4X4& mView, const MATRIX4X4& mProj, const VECTOR3& vLight,
                   const VECTOR3& vEye)
{
    // 使用するシェーダーの登録
